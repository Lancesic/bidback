...
const DONE_KEY = "bidback-hard-personalized-v1";
...
function activeProfile() {
  const pending = readJson<SignupProfile>(PENDING_KEY);
  if (pending?.name) return pending;
  const olderPending = readJson<SignupProfile>("bidback-pending-personalization-v2");
  if (olderPending?.name) return olderPending;
  const tester = readJson<TesterProfile>(TESTER_KEY);
  if (!tester?.name) return null;
  return tester;
}
...
export function SignupHardPersonalizer() {
  useEffect(() => {
    const originalSetItem = window.localStorage.setItem.bind(window.localStorage);
    window.localStorage.setItem = (key: string, value: string) => {
      if (key === APP_KEY) {
        const profile = activeProfile();
        if (profile?.name) return originalSetItem(key, personalizeValue(String(value), profile));
      }
