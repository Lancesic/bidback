...
type Estimate = EstimateForm & { id: string; estimateAmount: number; status: Status; followUps: FollowUp[]; createdAt: string; updatedAt: string };
type Tester = { id?: string; name: string; email: string; phone: string };
type CloudSignup = { ok?: boolean; configured?: boolean; tester?: Tester; appData?: { estimates?: Estimate[] } | null };
...
function buildFollowUps(estimate: Pick<Estimate, "customerName" | "jobType" | "estimateSentDate">) {
  return stages.map((day) => ({ id: makeId("fu"), stage: `Day ${day}`, dueDate: addDays(estimate.estimateSentDate, day), status: "Due" as StageStatus, messageContent: script(estimate, `Day ${day}`) }));
}
...
function appPackage(tester: Tester | null, estimates: Estimate[], comments = "") {
  ...
  return { submittedAt: new Date().toISOString(), tester, comments, summary, estimates };
}
...
export function BidBackApp() {
  ...
  useEffect(() => {
    ...
    const t = window.setTimeout(async () => {
      ...
      } catch { setCloudStatus("Local save"); }
    }, 800);
  ...
  const metrics = useMemo(() => {
    ...
    return {
      ...
      lostValue,
      openValue: pending.reduce((sum, e) => sum + e.estimateAmount, 0),
      due: pending.flatMap((e) => e.followUps).filter((f) => f.status === "Due" && f.dueDate <= today()).length,
  ...
  async function saveTester(event: FormEvent<HTMLFormElement>) {
    ...
    const profile = { name: String(fd.get("name") || "").trim(), email: String(fd.get("email") || "").trim(), phone: String(fd.get("phone") || "").trim() };
    try {
  ...
  function markStage(estimateId: string, stage: string, status: StageStatus) {
    setEstimates((current) => current.map((e) => e.id === estimateId ? { ...e, followUps: e.followUps.map((f) => f.stage === stage && f.status === "Due" ? { ...f, status, sentDate: status === "Sent" ? today() : f.sentDate, dueDate: status === "Snoozed" ? addDays(f.dueDate, 1) : f.dueDate } : f) } : e));
    show(status === "Sent" ? "Stage marked sent" : status);
  }
  function exportCsv() {
    const csv = [["Customer", "Phone", "Email", "Trade", "Job", "Amount", "Sent", "Status", "Notes"], ...estimates.map((e) => [e.customerName, e.phone, e.email, e.tradeType, e.jobType, String(e.estimateAmount), e.estimateSentDate, e.status, e.notes])].map((r) => r.map((c) => `"${c.replaceAll('"', '""')}"`).join(",")).join("\n");
    const a = document.createElement("a");
    a.href = URL.createObjectURL(new Blob([csv], { type: "text/csv" }));
    a.download = `bidback-estimates-${today()}.csv`;
    a.click();
  }
  async function openTallyFeedback(comments = "") {
    const pack = appPackage(tester, estimates, comments);
    await fetch("/api/feedback", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ testerId: tester?.id, comments, package: pack }) }).catch(() => undefined);
    const full = JSON.stringify(pack, null, 2);
    await copy(full);
    if (!TALLY_FEEDBACK_URL) { setFeedbackOpen(true); return; }
    ...
    url.searchParams.set("tester_id", tester?.id || "");
    url.searchParams.set("comments", comments);
    url.searchParams.set("summary", JSON.stringify(pack.summary));
    url.searchParams.set("app_data", `Saved in BidBack cloud when Supabase is configured. Local package copied to clipboard. ${full.slice(0, 5000)}`);
    window.open(url.toString(), "_blank", "noopener,noreferrer");
    show("Feedback copied. Tally opened.");
  }
  function sendEmailFallback(comments: string) {
    const pack = appPackage(tester, estimates, comments);
    const full = JSON.stringify(pack, null, 2);
    copy(full);
    location.href = `mailto:${FEEDBACK_EMAIL}?subject=${encodeURIComponent(`BidBack feedback from ${tester?.name || "tester"}`)}&body=${encodeURIComponent(`Tester: ${tester?.name || "Unknown"}\nEmail: ${tester?.email || "Unknown"}\nPhone: ${tester?.phone || "Unknown"}\n\nComments:\n${comments || "No comments entered."}\n\nSummary:\n${JSON.stringify(pack.summary, null, 2)}\n\nFull package copied to clipboard. Paste below if needed.\n\n${full.slice(0, 12000)}`)}`;
  }
...
function AddEstimate({ form, setForm, saveEstimate }: { form: EstimateForm; setForm: Dispatch<SetStateAction<EstimateForm>>; saveEstimate: (event: FormEvent) => void }) {
